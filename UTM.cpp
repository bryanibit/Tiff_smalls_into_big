...
FLOAT_ts DegToRad(FLOAT_ts deg) {
  return (deg / 180.0 * pi);
}
...
FLOAT_ts RadToDeg(FLOAT_ts rad) {
  return (rad / pi * 180.0);
}
...
FLOAT_ts ArcLengthOfMeridian (FLOAT_ts phi) {
  FLOAT_ts alpha, beta, gamma, delta, epsilon, n;
  FLOAT_ts result;
...
FLOAT_ts UTMCentralMeridian(int zone) {
  FLOAT_ts cmeridian;
  cmeridian = DegToRad(-183.0 + ((FLOAT_ts)zone * 6.0));
...
FLOAT_ts FootpointLatitude(FLOAT_ts y) {
  FLOAT_ts y_, alpha_, beta_, gamma_, delta_, epsilon_, n;
  FLOAT_ts result;
...
void MapLatLonToXY (FLOAT_ts phi, FLOAT_ts lambda, FLOAT_ts lambda0, FLOAT_ts &x, FLOAT_ts &y) {
    FLOAT_ts N, nu2, ep2, t, t2, l;
    FLOAT_ts l3coef, l4coef, l5coef, l6coef, l7coef, l8coef;
    //FLOAT_ts tmp; // Unused
...
void MapXYToLatLon (FLOAT_ts x, FLOAT_ts y, FLOAT_ts lambda0, FLOAT_ts& phi, FLOAT_ts& lambda)
{
  FLOAT_ts phif, Nf, Nfpow, nuf2, ep2, tf, tf2, tf4, cf;
  FLOAT_ts x1frac, x2frac, x3frac, x4frac, x5frac, x6frac, x7frac, x8frac;
  FLOAT_ts x2poly, x3poly, x4poly, x5poly, x6poly, x7poly, x8poly;
...
int LatLonToUTMXY (FLOAT_ts lat, FLOAT_ts lon, int zone, FLOAT_ts& x, FLOAT_ts& y) {
  if ( (zone < 1) || (zone > 60) )
...
// Returns:
// The function does not return a value.
void UTMXYToLatLon (FLOAT_ts x, FLOAT_ts y, int zone, bool southhemi, FLOAT_ts& lat, FLOAT_ts& lon) {
  FLOAT_ts cmeridian;
